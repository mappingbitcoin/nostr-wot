...
export function startNostrConnect(
    ...
    log("[NostrConnect] Starting connection to relay:", session.relay);
    log("[NostrConnect] Client pubkey:", session.clientPubkey);
    // Note: Never log secrets in production
    ...
    try {
        ...
        ws.onopen = () => {
            log("[NostrConnect] WebSocket connected to relay");
            ...
            log("[NostrConnect] Subscribing with filter:", JSON.stringify(filter));
            log("[NostrConnect] Waiting for events tagged with p:", session.clientPubkey);
            ws?.send(JSON.stringify(["REQ", subId, filter]));

            // Set connection timeout
            timeoutId = setTimeout(() => {
                if (!isConnected) {
                    log("[NostrConnect] Connection timeout after 2 minutes");
                    cleanup();
                    callbacks.onTimeout();
                }
            }, CONNECTION_TIMEOUT);
        ...
        ws.onmessage = async (event) => {
            try {
                ...
                log("[NostrConnect] Received message:", data[0], data[1] || "");

                if (data[0] === "EOSE") {
                    log("[NostrConnect] End of stored events, waiting for new events...");
                }

                if (data[0] === "OK") {
                    log("[NostrConnect] Event acknowledged by relay:", data[1], data[2] ? "success" : "failed", data[3] || "");
                }

                if (data[0] === "EVENT") {
                    const nostrEvent: NostrEvent = data[2];

                    log("[NostrConnect] Received NIP-46 event from:", nostrEvent.pubkey);
                    log("[NostrConnect] Event kind:", nostrEvent.kind);
                    // Note: Not logging tags, content, or decrypted data for security
                    ...
                    try {
                        ...
                        // Note: Not logging decrypted content for security
                        ...
                        if (isConnectResponse) {
                            ...
                            log("[NostrConnect] Successfully connected! Remote pubkey:", nostrEvent.pubkey);
                            cleanup();
                            callbacks.onConnected(nostrEvent.pubkey);
                        } else {
                            log("[NostrConnect] Response did not match connect criteria");
                        }
                    } catch (decryptErr) {
        ...
        ws.onclose = (event) => {
            log("[NostrConnect] WebSocket closed. Code:", event.code, "Reason:", event.reason);
            if (!isConnected) {
